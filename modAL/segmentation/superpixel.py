...
def get_superpixel(cfg: Union[DictConfig, ListConfig], img: Union[str, np.ndarray], max_superpixels: int = 1024):
    if isinstance(img, str):
        img = cv2.imread(img)

    algorithm = cfg.mining.superpixel_algorithm.lower()
    if algorithm == 'seeds':
        runner = SuperPixelSEEDS(img, num_superpixels=max_superpixels)
    elif algorithm in ['slic', 'slico', 'mslic']:
        height, width = img.shape[0:2]  # type: ignore
